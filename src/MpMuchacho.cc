...
void makeInto(msgpack_object &thisOne, cJSON *uno){
	switch(uno->type){
	case cJSON_Object:
	...
	default:
		assert(0);
		fprintf(stderr, "Unexpected type now for cJSON\n");
		abort();
		break;
	}
}
...
cJSON *buildFromUno(msgpack_object&thisOne){
	cJSON *newEle = NULL;
	switch (thisOne.type) {
	case MSGPACK_OBJECT_NIL:
...
cJSON *buildFromMpObj(msgpack_object_map obj) {
	...
	for(int i=0;i<l;++i){
		...
		cJSON *newEle = buildFromUno(thisKV.val);
...
cJSON *buildFromMpArr(msgpack_object_array arr){
	...
	for(int i=0;i<l;++i){
		msgpack_object &thisOne = *(arr.ptr+i);
		cJSON *newEle = buildFromUno(thisOne);
